...
const portfolioRelationshipArgsSchema = portfolioWorkspaceArgsSchema.extend({ sourceProjectId: z.string().trim().min(1), targetProjectId: z.string().trim().min(1), type: z.enum(["adaptation-of", "derived-from", "promotion-for", "companion-to", "supersedes", "informed", "validated", "invalidated", "produced"]) });
...
const portfolioResearchEnvelopeSchema = portfolioResearchBaseSchema.extend({ snapshot: z.record(z.string(), z.unknown()).optional(), evidence: z.record(z.string(), z.unknown()).optional(), evaluation: z.record(z.string(), z.unknown()).optional(), decision: z.record(z.string(), z.unknown()).optional() });
...
Use the portfolio.* affordances to inspect or initialize the current workspace Portfolio, manage projects and relationships, associate sessions, and register or list artifacts. A project whose kind is research owns longitudinal runs, immutable snapshots and observations, evaluations, and decisions through portfolio.research.*. Keep Portfolio metadata and research history synchronized when the user asks to create, organize, research, publish, promote, or track durable project work.
...
async function executePortfolio(id: string, rawArgs: unknown, context: OpenCodeContext): Promise<object> {
  ...
  if (id === "portfolio.research.snapshot.seal" || id === "portfolio.research.evidence.link" || id === "portfolio.research.evaluation.record" || id === "portfolio.research.decision.record") { const args = portfolioResearchEnvelopeSchema.parse(rawArgs); const workspace = await portfolioWorkspace(args.workspaceId, context); const key = id.endsWith("snapshot.seal") ? "snapshot" : id.endsWith("evidence.link") ? "evidence" : id.endsWith("evaluation.record") ? "evaluation" : "decision"; const body = args[key]; if (!body) throw new Error(`${key} is required`); const segment = key === "snapshot" ? "snapshots" : key === "evidence" ? "evidence" : key === "evaluation" ? "evaluations" : "decisions"; return { workspaceId: workspace.id, workspace: workspaceLabel(workspace), [key]: await serverJson("POST", portfolioPath(workspace.id, `/projects/${encodeURIComponent(args.projectId)}/research/${segment}`), body) }; }
