...
const { getCatalog } = await import("../../mcp/index.js")
const { createAgentMcpServer, registerAgentExecuteCapabilityTool } = await import("../../mcp/agent.js")
mock.restore()
...
class AmmWitness {
  ...
  async cancelRun(runId: string, context: { requestId: string }): Promise<AmmRunAcknowledgement> {
    this.cancelRunCalls += 1
    const run = await this.getRun(runId, context)
    const cancelled: AmmRunResponse = { ...run, state: "cancelled" }
    this.runs.set([...this.runs.entries()].find(([, candidate]) => candidate.runId === runId)?.[0] ?? "", cancelled)
    return acknowledgeRun(cancelled)
  }
...
describe("AMM native routes", () => {
  ...
  it("returns a recoverable Den operation ID when downstream start fails after reservation", async () => {
  ...
  it("reconciles a terminal idempotent start retry exactly once", async () => {
    ...
    expect((await firstRetry.json()) as unknown).toEqual((await secondRetry.json()) as unknown)
